...
Jae Kim의 개인 포트폴리오 웹사이트 — [jaekim.ai](https://jaekim.ai)에서 확인하세요.

---

## 주요 기능

- **캔버스 파티클 시스템** — 마우스 커서를 따라 반응하거나 흩어지는 파티클 효과 (데스크톱 전용)
- **커스텀 커서** — 시스템 커서 대신 캔버스로 그린 원형 커서, 인터랙티브 요소에 자석처럼 달라붙는 스냅 효과
- **마그네틱 내비게이션 닷** — 50px 반경 내 커서 자동 스냅; 클릭 시 사이드 패널 오픈
- **슬라이딩 사이드 패널** — 오른쪽에서 슬라이드로 열리며, 소개·이력·연락처 섹션이 순차적으로 페이드인
- **인터랙티브 경력 타임라인** — 가로형 점 타임라인, 방향키로 탐색 가능; 점 클릭 시 상세 역할 표시
- **플리퍼블 카드 덱** — 부채꼴로 펼쳐진 카드 형태의 "재미있는 사실들", 자개(나크레) 애니메이션 테두리와 함께 클릭 시 뒤집힘
- **리퀴드 글래스 UI** — 백드롭 블러가 적용된 반투명 유리 느낌의 디테일 팝업
- **서명 워터마크** — 패널 열림 여부에 따라 시각적으로 중앙을 유지하며 이동하는 서명 이미지
- **이메일 토글** — "Email" 클릭 시 주소 표시/숨김 전환 (기본적으로 mailto 링크 없음)
- **테마 시스템** — 다크, 라이트, 다크 샌드 세 가지 프리셋 전환 가능
- **반응형 디자인** — 모바일에서 전체 너비 패널 및 터치 친화적 인터랙션; 터치 기기에서는 파티클·커스텀 커서 비활성화
- **부드러운 로딩** — 폰트와 이미지 로드 완료 후 페이지 페이드인
